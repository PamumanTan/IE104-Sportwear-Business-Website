...
def rename_path(path: str):
    ...
    for i in range(1, len(old_name)):
        ...
        if c == "_" or c == " ":  # special characters except hyphen '-'
            ...
            if old_name[i - 1] != "-" and not (
                old_name[i - 1] == "_" or old_name[i - 1] == " "
            ):
    ...
    # add suffix
    new_name = "nn_" + new_name

    # remove suffix
    # new_name = new_name.replace("nn_", "")
...
def rename_tree(path: str):
    ...
    if path.split(sep="/").pop()[0] != ".":  # not a system folder
        if os.path.isdir(path):  # is a folder
            for i in os.listdir(
                path
            ):  # loop through a list of files and folders in the directory
                rename_tree(path + "/" + i)
